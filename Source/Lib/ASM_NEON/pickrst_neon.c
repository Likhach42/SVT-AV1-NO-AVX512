    ...
    if (params->r[0] > 0 && params->r[1] > 0) {
        ...
        do {
            ...
            for (int k = j; k < width; ++k) {
                int32_t u = (dat8[k] << SGRPROJ_RST_BITS);
                int32_t v = (1 << (SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS - 1));
                v += xq[0] * flt0[k] + xq[1] * flt1[k] - u * (xq[0] + xq[1]);
                int32_t e = (v >> (SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS)) + dat8[k] - src8[k];
                sse += e * e;
            }
        ...
        do {
            ...
            for (int k = j; k < width; ++k) {
                ...
                int32_t e = ROUND_POWER_OF_TWO(v, SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS) + dat8[k] - src8[k];
                sse += e * e;
            }
    ...
    if (params->r[0] > 0 && params->r[1] > 0) {
        ...
        do {
            ...
            for (int k = j; k < width; ++k) {
                ...
                v -= (xq[1] + xq[0]) * (int32_t)(dat[k] << SGRPROJ_RST_BITS);
                int32_t e = (v >> (SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS)) + dat[k] - src[k];
                sse += (int64_t)e * e;
            }
        ...
        do {
            ...
            for (int k = j; k < width; ++k) {
                int32_t v = 1 << (SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS - 1);
                v += xq_active * (int32_t)((uint32_t)flt[k] - (uint16_t)(dat[k] << SGRPROJ_RST_BITS));
                const int32_t e = (v >> (SGRPROJ_RST_BITS + SGRPROJ_PRJ_BITS)) + dat[k] - src[k];
                sse += (int64_t)e * e;
            }
